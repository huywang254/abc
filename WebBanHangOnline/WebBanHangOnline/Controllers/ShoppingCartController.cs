...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
            ...
            if (Request.QueryString.Count > 0)
            {
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00" && vnp_TransactionStatus == "00")
                    {
                        ...
                        if (itemOrder != null)
                        {
                            itemOrder.Status = 2;//da thanh toan
        ...
        public ActionResult CheckOut (OrderViewModel reg)
        {
            ...
            if(ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    order.CustomerName = reg.CustomerName;
                    ...
                    order.TypePayment = reg.TypePayment;
                    order.CreatedDate = DateTime.Now;
                    order.ModifiedDate = DateTime.Now;
                    order.CreatedBy = reg.Phone;
                    ...
                    TongTien = thanhtien;
                    string contentCustomter = System.IO.File.ReadAllText(Server.MapPath("~/Content/template/send2.html"));
                    contentCustomter = contentCustomter.Replace("{{MaDon}}", order.Code);
                    contentCustomter = contentCustomter.Replace("{{SanPham}}", strSanPham);
                    contentCustomter = contentCustomter.Replace("{{NgayDat}}", DateTime.Now.ToString("dd/MM/yyyy"));
                    contentCustomter = contentCustomter.Replace("{{TenKhachHang}}", order.CustomerName);
                    contentCustomter = contentCustomter.Replace("{{Phone}}", order.Phone);
                    contentCustomter = contentCustomter.Replace("{{Email}}", reg.Email);
                    contentCustomter = contentCustomter.Replace("{{DiaChiNhanHang}}", order.Address);
                    contentCustomter = contentCustomter.Replace("{{ThanhTien}}", WebBanHangOnline.Common.Common.FormatNumber(thanhtien,0));
                    contentCustomter = contentCustomter.Replace("{{TongTien}}", WebBanHangOnline.Common.Common.FormatNumber(TongTien, 0));
                    WebBanHangOnline.Common.Common.SendMail("ShopOnline", "Đơn Hàng #" + order.Code, contentCustomter.ToString(), reg.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{Email}}", reg.Email);
                    ...
                    cart.ClearCart();
                    code = new { Success = true, Code = reg.TypePayment, Url = "" };
                    if (reg.TypePayment == 2)
                    {
                        var url = UrlPayment(reg.TypePaymentVN,order.Code); 
                        code = new { Success = true, Code = reg.TypePayment, Url = url };
                    }
